...
pnpm workspace, three packages ([plan.md](./plan.md) § Source Code):

- `packages/core/` — grammar, model, ledger, measure, verdict, `scripts/measure.py`
- `packages/mcp/` — the streamable-HTTP MCP server of irreversible actions
...
**A** — critical path: `apps/bench`, `packages/core/src/measure`, `packages/core/src/verdict`.
**B** — `packages/mcp`, `packages/core/src/ledger`, OpenUI card, docket.
...
- [ ] T001 Create the pnpm workspace root: `pnpm-workspace.yaml` listing `packages/*` and `apps/*`; root `package.json` (ESM, `"type": "module"`, `packageManager: pnpm@9`) with the scripts `demo`, `test`, `lint`, `build`, `seed`; `.gitignore` covering `.env*`, `node_modules/`, `.crossexam/`; `.nvmrc` pinning Node 22.14
- [ ] T002 Create the three package manifests — `packages/core/package.json`, `packages/mcp/package.json`, `apps/bench/package.json` — each with `"exports": "./src/index.ts"` (source export, no emit) and the exact pins of research [D-02](./research.md): `@truefoundry/trueforge` 0.1.4, `@truefoundry/trueforge-sdk` 0.1.3, `@modelcontextprotocol/sdk` 1.30.0, `zod` 4.5.2, `tsx` 4.23.12, `vitest` 4.1.11, `typescript` 5.9.3 — no caret ranges anywhere (Risk R9)
- [ ] T003 [P] Create `tsconfig.base.json` at the repo root and a `tsconfig.json` in each of the three packages; wire `pnpm build` to a workspace-wide `tsc --noEmit` so it fails on a real type error ([D-01](./research.md))
...
- [ ] T020 [P] [US1] Implement the streamable-HTTP MCP server in `packages/mcp/src/server.ts` with the entrypoint `packages/mcp/src/index.ts`, exposing `bulk_refund`, `issue_payout`, and `close_account` with the arguments and `destructive: true` annotations of [contracts/mcp-tools.md](./contracts/mcp-tools.md); handlers return the proposal encoded in the emoji grammar and **execute nothing at proposal time** (FR-001)
...
- [ ] T025 [US1] Implement the correlation in `apps/bench/src/correlate/index.ts`: `tool.approval_required` carries only `{id, source_event_id}`, so walk back to the preceding `model.message` to recover the tool name and its full arguments (FR-002; the harness does not do this — [research.md](./research.md) §A)
...
- [ ] T027 [P] [US1] Write the Evaluator prompt in `apps/bench/src/prompts/evaluator.ts`: it receives the charge sheet, measures via the executor, and returns its verdict in the emoji grammar with `🧮`/`💰`/`♻` present on every `⚖allow` and `⚖deny`, and a `📝` reason on a `deny` carrying the measured figures (FR-009, FR-012)
- [ ] T028 [P] [US1] Write the target agent prompt in `apps/bench/src/prompts/target.ts`: an ordinary support agent that proposes `bulk_refund` with its declared `🔢` and `💵` figures, and on reading a denial reason narrows its own selection criteria and proposes again — the correction comes from the reason, not from a script (FR-015, US1-3)
- [ ] T029 [US1] Implement verdict resolution in `apps/bench/src/sessions/resolve.ts` mapping `allow` → resolve approval allow, `deny` → resolve with `reason` = the `📝` text, `escalate` → leave the approval pending; enforce the double-decision guard as a compare-and-set on `case_id` so a second decision on a resolved case is rejected and the first stands (FR-012, FR-013, FR-014, [data-model.md](./data-model.md) §10)
- [ ] T030 [US1] Wire the whole loop in `apps/bench/src/demo.ts` behind `pnpm demo`, emitting the observable trace of [quickstart.md](./quickstart.md) Scenario 1 — proposal, measurement with its executor and duration, verdict with its rule number, re-proposal, second measurement, final decision, execution — and reporting a divergence rather than pretending the scripted numbers were measured if the agent leaves the seeded path (FR-016, spec § Edge Cases)
...
- [ ] T037 [US2] Implement the four checks in `packages/mcp/src/guardrails.ts` — $2,000 per-action ceiling, per-customer frequency cap, eligibility-policy check, and a self-reported confidence of 0.94 — evaluated inside the tool handler before it returns its proposal, all four passing correctly on the damaging proposal; four hardcoded checks, deliberately **not** a rules engine ([D-13](./research.md), FR-017, FR-018)
- [ ] T038 [US2] Attach the `GuardrailReport` to the tool's proposal payload in `packages/mcp/src/tools.ts`, carry it through the charge sheet in `apps/bench/src/correlate/chargeSheet.ts`, render the four pass/fail lines in the `apps/bench/src/demo.ts` trace, and add the `--guardrails-only` flag that stops after them (FR-018)
