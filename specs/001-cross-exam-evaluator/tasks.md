...
- [ ] T017a [US1] Create `packages/measure` (`@crossexam/measure`, manifest with the D-02 pins and `@crossexam/core` as a workspace dependency, a `tsconfig.json` extending `tsconfig.base.json` so `pnpm build` and `pnpm lint` cover it) — a streamable-HTTP MCP server exposing the read-only `measure` tool of [research.md](./research.md) D-15: string arguments `criteria` and `table`, runs T017's resolution order, reads only `CROSSEXAM_REPLICA_PATH`, listens on `CROSSEXAM_MEASURE_SERVER_URL`, returns the three grammar lines as text and `{executor, duration_ms, script_sha256, criteria, table}` as `structuredContent`; non-destructive, so no approval. Verify with a real call over HTTP against `fixtures/replica.json` and paste the result (FR-004, FR-005, D-15)

### The verdict guardrail — the only code that can force `escalate` (owner A)

- [ ] T018 [P] [US1] **(needs only T007 + T008 — start during the Phase 2 window)** Implement the guardrail `decide(proposal, evaluatorVerdict, observed, config): Verdict` in `packages/core/src/verdict/decide.ts` as the six ordered rules of [research.md](./research.md) D-06 — every rule can only force `escalate`; if none fires the Evaluator's verdict stands with `observed` as evidence and `rule: 6`. Code never emits `allow` or `deny` on its own (Constitution II, FR-008–FR-012)
- [ ] T019 [US1] Write the verdict unit suite in `packages/core/tests/verdict.test.ts` covering each rule and their **ordering**: threshold exceeded plus a mismatch → `escalate` under rule 3; an Evaluator `deny` citing figures that differ from `observed` → rule 4; an Evaluator `allow` with `observed.duplicates > 0` → rule 5; a matching `deny` and a matching `allow` both stand under rule 6; `observed === null`, or `observed` echoing a `criteria`/`table` other than the proposal's → rule 2 whatever the Evaluator wrote; the invariant `verdict !== 'escalate' ⇒ evidence !== null` (Constitution II, [data-model.md](./data-model.md) §9)
...
- [ ] T029 [US1] Implement verdict resolution in `apps/bench/src/sessions/resolve.ts`: decode the Evaluator's verdict, read `observed` — the last `measure` tool result of its turn, run `decide()` (T018), then map the result — `allow` → resolve approval allow, `deny` → resolve with `reason` = the `📝` text, `escalate` → leave the approval pending; enforce the double-decision guard by compare-and-set on `case_id` in the in-memory case table ([data-model.md](./data-model.md) §10) (FR-013, FR-014, FR-015)
