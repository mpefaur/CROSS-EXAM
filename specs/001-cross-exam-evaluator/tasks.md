...
- [ ] T008a [P] **(needs only T001–T002)** Patch the harness so grammar lines are tool calls: `pnpm patch @truefoundry/trueforge@0.1.4`, and in `dist/main.js` wrap the `new VercelAILLM(...)` of the `llm:` factory in a `GrammarToolCallLLM` per [research.md](./research.md) D-14 — strip `body.tools` on the way in; on the final message synthesise `tool_calls` and `finish_reason: "tool_calls"` from a `🧾` line — then `pnpm patch-commit` and commit `patches/` plus the `patchedDependencies` entry. `trueforge-core` is not touched. Verify with a real run: start the harness from the workspace (`pnpm exec trueforge`, never `npx`, so the patched package loads), attach an MCP server, have the model emit the four proposal lines as text, and paste the `tool.approval_required` event naming the tool; also paste one Claude turn whose context already holds a tool result, to confirm or trigger R-14a (FR-001, FR-024, D-14)
