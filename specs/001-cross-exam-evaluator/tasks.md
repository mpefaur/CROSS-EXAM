...
- [ ] T024 [US1] Implement the SSE consumer in `apps/bench/src/sessions/stream.ts`, indexing every event by `id` so a later event can reach the one it references — the Bench reads what the harness held and what the tool returned from this index (FR-001, FR-004)
